...
Now, you'll notice some familiar keywords in there, in particular, '*', `body`, `h1`, `p`, and `ol`.  That's right, they're the tags we know and love!  But, in CSS, these are called *selectors*.  The selector tells us what tag you're about to style.  Star (*) is the universal selector it selects evey element in html So, whatever code you put in between the curly braces `{}` after the '*' selector will affect everything on the page. Whatever you put in the braces after the `body` selector will affect everything in the `<body>` tags.  Whatever you put in the braces after the `p` selector will affect what's in the `<p>` tags.
